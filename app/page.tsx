  ...
  return <main className="min-h-screen bg-[#05070d] grid-bg text-slate-100 overflow-x-hidden">
    ...
    <section className="pl-[250px] min-h-screen">
      ...
      <div className="p-5">
        ...
        {active==='research' && <MarketIntelligence live={live} terminal={terminal} coins={coins} news={news} signals={signals} symbol={symbol}/>}
        {active==='reports' && <AIReports aiQ={aiQ} setAiQ={setAiQ} ask={ask} asking={asking} aiA={aiA} news={news} terminal={terminal} signals={signals}/>} 
...
function MarketIntelligence({live,terminal,coins,news,signals,symbol}:any){
  const price = terminal?.ticker?.price;
  const book = terminal?.orderBook || {bids:[], asks:[]};
  const bids = book.bids || [];
  const asks = book.asks || [];
  const bestBid = bids[0]?.price;
  const bestAsk = asks[0]?.price;
  const spread = bestBid && bestAsk ? ((Number(bestAsk)-Number(bestBid))/Number(price||bestBid)*100) : null;
  const bidDepth = bids.slice(0,20).reduce((a:any,b:any)=>a+Number(b.total||0),0);
  const askDepth = asks.slice(0,20).reduce((a:any,b:any)=>a+Number(b.total||0),0);
  const gainers = [...(coins||[])].filter((c:any)=>Number.isFinite(Number(c.price_change_percentage_24h))).sort((a:any,b:any)=>Number(b.price_change_percentage_24h)-Number(a.price_change_percentage_24h)).slice(0,8);
  const losers = [...(coins||[])].filter((c:any)=>Number.isFinite(Number(c.price_change_percentage_24h))).sort((a:any,b:any)=>Number(a.price_change_percentage_24h)-Number(b.price_change_percentage_24h)).slice(0,8);
  return <div className="grid grid-cols-12 gap-4">
    <Card className="col-span-12"><div className="flex flex-wrap items-start justify-between gap-4"><div><h2 className="text-3xl font-black text-gradient">Market Intelligence</h2><p className="text-slate-400 mt-1">Live market structure from SoDEX plus SoSoValue research context. No AI report generation here.</p></div><div className="pill rounded-full px-4 py-2 text-sm">{symbol} · {terminal?.updatedAt?new Date(terminal.updatedAt).toLocaleString():'live sync pending'}</div></div></Card>
    <MetricCard label="Current Price" value={money(price,false)} note={terminal?.sodex?.source==='SoDEX'?'SoDEX mark / route':'live market route'}/>
    <MetricCard label="Spread" value={spread===null?'Unavailable':`${spread.toFixed(4)}%`} note="Best ask - best bid"/>
    <MetricCard label="Bid Depth 20" value={bidDepth?money(bidDepth):'Unavailable'} note="Top 20 levels"/>
    <MetricCard label="Ask Depth 20" value={askDepth?money(askDepth):'Unavailable'} note="Top 20 levels"/>
    <Card className="col-span-12 xl:col-span-4"><h3 className="text-xl font-black mb-3">Liquidity Snapshot</h3><div className="grid gap-3"><MetricTiny label="Order book source" value={book.source || 'Unavailable'}/><MetricTiny label="Bid levels" value={bids.length}/><MetricTiny label="Ask levels" value={asks.length}/><MetricTiny label="Recent trades" value={(terminal?.trades||[]).length}/><MetricTiny label="Market universe" value={(coins||[]).length}/></div></Card>
    <Card className="col-span-12 xl:col-span-4"><h3 className="text-xl font-black mb-3">Top Movers</h3>{gainers.length?gainers.map((c:any)=><CoinMini key={c.id} c={c}/>):<Empty title="No live movers" text="Market provider returned no mover data."/>}</Card>
    <Card className="col-span-12 xl:col-span-4"><h3 className="text-xl font-black mb-3">Risk Movers</h3>{losers.length?losers.map((c:any)=><CoinMini key={c.id} c={c}/>):<Empty title="No risk movers" text="Market provider returned no downside data."/>}</Card>
    <Card className="col-span-12 xl:col-span-7"><h3 className="text-xl font-black mb-3">SoDEX Depth Map</h3><ResponsiveContainer height={320}><BarChart data={[...asks.slice(0,15).reverse().map((x:any)=>({...x,side:'Ask'})),...bids.slice(0,15).map((x:any)=>({...x,side:'Bid'}))]}><XAxis dataKey="price" tick={{fontSize:10}}/><YAxis/><Tooltip contentStyle={{background:'#091122',border:'1px solid #263759'}}/><Bar dataKey="total" fill="#22c55e"/></BarChart></ResponsiveContainer></Card>
    <Card className="col-span-12 xl:col-span-5"><h3 className="text-xl font-black mb-3">SoSoValue Research Pulse</h3>{news?.length?news.slice(0,5).map((n:any,i:number)=><div key={i} className="py-3 border-b border-slate-800"><span className="text-xs text-violet-300">LIVE RESEARCH</span><h4 className="font-black">{n.title||n.newsTitle||n.content||'Live research item'}</h4></div>):<Empty title="No live research" text="SoSoValue returned no research items."/>}</Card>
  </div>
}
function MetricCard({label,value,note}:any){ return <Card className="col-span-12 md:col-span-6 xl:col-span-3"><p className="text-xs uppercase text-slate-400">{label}</p><h3 className="text-2xl font-black mt-2">{value}</h3><p className="text-xs text-cyan-200/70 mt-2">{note}</p></Card> }
function CoinMini({c}:any){ return <div className="flex items-center gap-3 py-2 border-b border-slate-800"><img src={c.image||''} className="h-8 w-8 rounded-full"/><div className="flex-1"><b>{c.symbol?.toUpperCase()}</b><p className="text-xs text-slate-400">{c.name}</p></div><div className="text-right"><b>{money(c.current_price,false)}</b><p className={Number(c.price_change_percentage_24h)>=0?'text-emerald-300 text-xs':'text-rose-300 text-xs'}>{pct(c.price_change_percentage_24h)}</p></div></div> }
function AIReports({news,aiQ,setAiQ,ask,asking,aiA,terminal,signals}:any){
  const templates = [
    'Generate a Daily Market Report using current SoDEX market structure and SoSoValue news.',
    'Create a BTC trade setup with liquidity, order book, risk guard, TP/SL ideas and execution caveats.',
    'Summarize SoSoValue news and explain possible impact on BTC, ETH and SOL.',
    'Create a risk report for the current futures setup. Include leverage warnings and max notional controls.'
  ];
  return <div className="grid grid-cols-12 gap-4">
    <Card className="col-span-12 xl:col-span-5"><h3 className="text-2xl font-black mb-2">AI Reports</h3><p className="text-slate-400 mb-4">Generate structured research reports from live SoSoValue news, SoDEX order book, trades and signal context.</p><div className="grid gap-2 mb-4">{templates.map(t=><button key={t} onClick={()=>setAiQ(t)} className="pill rounded-2xl p-3 text-left text-sm hover:border-violet-500">{t}</button>)}</div><textarea value={aiQ} onChange={e=>setAiQ(e.target.value)} className="input h-36"/><button onClick={ask} className="rounded-2xl bg-gradient-to-r from-violet-600 to-cyan-500 px-5 py-3 font-black">{asking?'Generating report...':'Generate AI Report'}</button></Card>
    <Card className="col-span-12 xl:col-span-7"><div className="flex justify-between items-center mb-3"><h3 className="text-xl font-black">Report Output</h3><span className="pill rounded-full px-3 py-1 text-xs">Provider: AI_API_KEY / ChainOpera router</span></div><pre className="no-data rounded-2xl p-4 whitespace-pre-wrap min-h-[520px] max-h-[720px] overflow-auto">{aiA||'Choose a template or write a prompt. The report uses live SoDEX + SoSoValue context only. If data is unavailable, the report must say unavailable.'}</pre></Card>
    <Card className="col-span-12 xl:col-span-6"><h3 className="text-xl font-black mb-3">Context Loaded</h3><MetricTiny label="Order book levels" value={(terminal?.orderBook?.bids||[]).length + (terminal?.orderBook?.asks||[]).length}/><MetricTiny label="Recent trades" value={(terminal?.trades||[]).length}/><MetricTiny label="SoSoValue items" value={(news||[]).length}/><MetricTiny label="Signals" value={(signals||[]).length}/></Card>
    <div className="col-span-12 xl:col-span-6"><NewsFeed news={(news||[]).slice(0,4)}/></div>
  </div>
}

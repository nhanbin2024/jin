...
export default function Page(){
  ...
  return <main className="min-h-screen bg-[#05070d] grid-bg text-slate-100 overflow-x-hidden">
    <aside className="fixed inset-y-0 left-0 w-[250px] bg-[#070b15]/95 border-r border-slate-800 p-4 overflow-y-auto z-30">
      <div className="flex items-center gap-3 mb-7"><img src="/sosovalue.png" alt="SoSoValue" className="h-14 w-14 rounded-2xl object-cover shadow-3d bg-slate-950"/><div><h1 className="text-2xl font-black">QuyNhon AI</h1><p className="text-xs text-cyan-100/60">SoSoValue × SoDEX OS</p></div></div>
...
function TradingViewFallback({symbol,height=330}:any){
  const tvSymbol = `BINANCE:${String(symbol||'BTCUSDT').toUpperCase()}`;
  const html = `<!doctype html><html><head><meta charset="utf-8"><style>html,body,#tv{margin:0;width:100%;height:100%;background:#07101f;overflow:hidden}</style></head><body><div id="tv"></div><script src="https://s3.tradingview.com/tv.js"></script><script>new TradingView.widget({autosize:true,symbol:'${tvSymbol}',interval:'15',timezone:'Etc/UTC',theme:'dark',style:'1',locale:'en',hide_top_toolbar:false,hide_legend:false,allow_symbol_change:false,container_id:'tv',studies:['Volume@tv-basicstudies']});</script></body></html>`;
  return <iframe title="Live TradingView chart" className="w-full rounded-2xl border border-slate-800 bg-slate-950" style={{height}} srcDoc={html}/>;
}
function MiniChart({data,height=330,symbol}:any){ const d=(data||[]).map((x:Candle)=>({...x, label:new Date(x.time).toLocaleTimeString([], {hour:'2-digit',minute:'2-digit'})})); if(!d.length) return <TradingViewFallback symbol={symbol} height={height}/>; return <ResponsiveContainer width="100%" height={height}><ComposedChart data={d}><CartesianGrid stroke="#18233b"/><XAxis dataKey="label" stroke="#71809f" minTickGap={28}/><YAxis stroke="#71809f" domain={['auto','auto']}/><Tooltip contentStyle={{background:'#091122',border:'1px solid #283b65'}}/><Area type="monotone" dataKey="close" stroke="#20e0a4" fill="#0f766e55" strokeWidth={2}/><Bar dataKey="volume" fill="#334155" yAxisId="vol" opacity={0.35}/><YAxis yAxisId="vol" hide/></ComposedChart></ResponsiveContainer>; }
